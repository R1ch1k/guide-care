...
async def start_conversation(payload: ConversationCreate, db: AsyncSession = Depends(get_async_session)):
    ...
    conv = await crud.start_conversation(db, payload.patient_id, payload.selected_guideline)
    return conv

@router.get("/{conv_id}", response_model=ConversationOut)
async def get_conversation(conv_id: str, db: AsyncSession = Depends(get_async_session)):
